...
async function fetchRegressionData(): Promise<RegressionData> {
  // Try the live API first (has real-time ownership + fresh data)
  try {
    const res = await fetch(`${API_BASE}/season/regression`, { cache: "no-store" })
    if (res.ok) return RegressionDataSchema.parse(await res.json())
  } catch {
    // fall through to static
  }
  // Fall back to the static export generated by the daily GitHub Actions workflow
  const res = await fetch(`${BASE_PATH}/api/regression.json`)
  if (!res.ok) throw new Error("No regression data available")
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        {error && (
          <div className="mb-6 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800 dark:border-red-900 dark:bg-red-950/30 dark:text-red-400">
            {error instanceof Error ? error.message : "Failed to fetch"}. Static data may not have been generated yet — run the dashboard export script.
          </div>
        )}
