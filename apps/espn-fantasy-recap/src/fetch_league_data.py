...
class FantasyDataFetcher:
    ...
    def __init__(
        self, league_id: int, year: int, espn_s2: str = None, swid: str = None
    ):
        """
    ...
    def connect(self) -> Optional[League]:
        ...
        try:
            ...
            print(
                f"✅ Connected to league: {self.league.name if hasattr(self.league, 'name') else 'Unknown'}"
            )
            return self.league
        except Exception as e:
            print(f"❌ Error connecting to league: {e}")
            print(
                "Note: If league is private, you'll need to add espn_s2 and swid cookies"
            )
            return None
    ...
    def generate_matchups_markdown(self, week: int) -> str:
        ...
        for i, matchup in enumerate(matchups, 1):
            ...
            output.append(
                f"**Final Score:** {matchup.home_team.team_name} {matchup.home_score:.2f} - "
            )
            output.append(f"{matchup.away_score:.2f} {matchup.away_team.team_name}  \n")
            output.append(
                f"**Records:** {matchup.home_team.team_name} ({matchup.home_team.wins}-{matchup.home_team.losses}-{matchup.home_team.ties}) | "
            )
            output.append(
                f"{matchup.away_team.team_name} ({matchup.away_team.wins}-{matchup.away_team.losses}-{matchup.away_team.ties})  \n"
            )
            ...
            for player in matchup.home_lineup:
                if player.slot_position not in ["BE", "IR"]:
                    ...
                    output.append(
                        f"| {player.slot_position} | {player.name} | {pro_team} | {proj:.1f} | {actual:.1f} |\n"
                    )
            ...
            for player in matchup.home_lineup:
                if player.slot_position == "BE":
                    ...
                    pro_team = getattr(player, "proTeam", "FA")
                    output.append(
                        f"| {player.name} | {pro_team} | {proj:.1f} | {actual:.1f} |\n"
                    )
    ...
    def generate_standings_markdown(self) -> str:
        ...
        for i, team in enumerate(standings, 1):
            ...
            output.append(
                f"| {i} | {team.team_name} | {owner} | {team.wins} | {team.losses} | "
            )
            output.append(
                f"{team.ties} | {pct:.3f} | {team.points_for:.1f} | {team.points_against:.1f} | {streak} |\n"
            )
        ...
        output.append(
            f"- **Most Points For:** {top_scorer.team_name} ({top_scorer.points_for:.1f} PF)\n"
        )

        # Find best defense
        best_defense = min(standings, key=lambda x: x.points_against)
        output.append(
            f"- **Fewest Points Against:** {best_defense.team_name} ({best_defense.points_against:.1f} PA)\n"
        )
        ...
        output.append(
            f"- **Best Point Differential:** {best_diff.team_name} (+{diff:.1f})\n"
        )
        ...
        output.append(
            f"- **Most Transactions:** {most_moves.team_name} ({moves} moves)\n"
        )
    ...
    def calculate_week_stats(self, week: int) -> Dict:
        ...
        for m in matchups:
            # Skip bye week matchups where away_team or home_team is not a team object
            # This happens in playoff weeks when top seeds have byes
            if not hasattr(m.away_team, "team_name") or not hasattr(
            ...
            home_bench_points = sum(
                p.points for p in m.home_lineup if p.slot_position == "BE"
            )
            away_bench_points = sum(
                p.points for p in m.away_lineup if p.slot_position == "BE"
            )

            if (
                not stats["most_bench_points"]
                or home_bench_points > stats["most_bench_points"]["points"]
            ):
            ...
            if (
                not stats["most_bench_points"]
                or away_bench_points > stats["most_bench_points"]["points"]
            ):
                stats["most_bench_points"] = {
    ...
    def generate_week_summary(self, week: int) -> str:
        ...
        if stats.get("highest_score"):
            output.append(
                f"**👑 Week Winner:** {stats['highest_score'][0]} ({stats['highest_score'][1]:.1f} points)\n\n"
            )

        if stats.get("lowest_score"):
            output.append(
                f"**🗑️ Dumpster Fire:** {stats['lowest_score'][0]} ({stats['lowest_score'][1]:.1f} points)\n\n"
            )

        if stats.get("biggest_blowout"):
            b = stats["biggest_blowout"]
            output.append(
                f"**💥 Biggest Blowout:** {b['winner']} destroyed {b['loser']} by {b['margin']:.1f} points\n\n"
            )

        if stats.get("closest_game"):
            c = stats["closest_game"]
            output.append(
                f"**😰 Nail Biter:** {c['team1']} vs {c['team2']} decided by {c['margin']:.1f} points\n\n"
            )

        if stats.get("most_bench_points"):
            bp = stats["most_bench_points"]
            output.append(
                f"**🪑 Bench Points Champion:** {bp['team']} left {bp['points']:.1f} points on the bench\n\n"
            )
...
def main():
    ...
    if stats.get("highest_score"):
        print(
            f"🚀 Highest Score: {stats['highest_score'][0]} - {stats['highest_score'][1]:.1f} pts"
        )

    if stats.get("lowest_score"):
        print(
            f"💩 Lowest Score: {stats['lowest_score'][0]} - {stats['lowest_score'][1]:.1f} pts"
        )

    if stats.get("biggest_blowout"):
        b = stats["biggest_blowout"]
        print(
            f"💥 Biggest Blowout: {b['winner']} beat {b['loser']} by {b['margin']:.1f}"
        )
